 ...
 *    - 在设置语言及编码之后。随后对GME_GETTEXT(str)或者__(string)的调用,如果string对应的翻译存在,则返回翻译文本，否则会返回string。
...
/**
 * @brief           国际化支持.
 *                  缺省是线程安全的。如果不需要线程安全，可以定义宏GME_NOTHREADSAFE并以--disable-threads选项重新编译ICU库。
 * @todo cmake support GME_NOTHREADSAFE configuration.
 **/
...
public:
     ...
     * @return          返回翻译后的结果.翻译后的结果本线程内下一次调用gettext之前是有效的。
     * @exception
     **/
     ...
     * @detail          无论是否设置GME_NOTHREADSAFE,本函数不是线程安全的，只应该在开始时调用一次。中途如果调用，也应该由调用者维护其线程安全性。
     * @param local     [IN], 指定local的字符串,如果给定NULL,则采用系统当前locale.
     * @param encoding  [IN], 指定编码,如果给定NULL,则采用系统当前encoding.(gme use utf-8 always,don't change it!)
...
static  inline  const char* GME_GETTEXT(const char* str)
{
    return gme::I18n::instance().gettext(str);
}

#define __(str)	GME_GETTEXT(str)


#endif //GME_UTILS_I18N_H
