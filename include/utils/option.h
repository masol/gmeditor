...
#ifndef GME_UTILS_OPTION_H
#define GME_UTILS_OPTION_H
...
namespace gme{
    ...
    public:
         ...
         **/
        typedef boost::function<bool (const std::string&, boost::any&, Act_Type)>    slottype;
    ...
    public:
		...
		**/
         ...
         * @param path      [IN], 属性的路径
         * @return          返回属性的值。
         * @exception       如果所指路径不存在，throw ptree_bad_path, 如果类型转化失败， throw boost::bad_any_cast
         **/
        ...
        /**
         * @brief           给一个属性添加一个监视functor，当该属性的值发生改变的时候，functor会被调用
     ...
     **/
    template <>
    inline boost::any Option::get<boost::any>(const std::string & path)
    {
#ifndef GME_NOTHREAD_SAFE
        boost::shared_lock<boost::shared_mutex> l(m_mutex);
#endif
     ...
     **/
    template<>
    inline boost::optional<boost::any> Option::get_optional<boost::any>(const std::string& path)
    {
#ifndef GME_NOTHREAD_SAFE
        boost::shared_lock<boost::shared_mutex> l(m_mutex);
#endif
